...
function Masthead() {
  return () => (
    <section aria-label="Welcome" mix={mastheadCss}>
      <p mix={eyebrowCss}>Welcome to</p>
      <RemixWordmarkHero />
    </section>
  )
}
...
function Columns() {
  return () => (
    <section aria-label="Getting started" mix={columnsCss}>
      <GetStartedCard />
      <CodingWithAiCard />
    </section>
  )
...
function GetStartedCard() {
  return () => (
    <div mix={cardLeftCss}>
      <h2 mix={cardHeadingCss}>Get started</h2>
      <ul mix={linkListCss}>
        <li>
...
function CodingWithAiCard() {
  return () => (
    <div mix={cardRightCss}>
      <h2 mix={cardHeadingCss}>Coding with AI?</h2>
      <div mix={cardBodyCss}>
        <p mix={cardDescriptionCss}>
...
function CardLink(handle: Handle<{ href: string; icon: RemixNode; label: string }>) {
  return () => {
    ...
    return (
      <a href={href} mix={cardLinkCss}>
        <IconSlot>{icon}</IconSlot>
        <span mix={cardLinkLabelCss}>{label}</span>
      </a>
    )
  }
}

function IconSlot(handle: Handle<{ children: RemixNode }>) {
  return () => {
    let { children } = handle.props
    return (
      <span aria-hidden="true" mix={iconSlotCss}>
        {children}
      </span>
    )
  }
...
function PageFooter() {
  return () => (
    <footer mix={pageFooterCss}>
      <div mix={pageFooterInnerCss}>
        <FooterWordmark />
        <nav aria-label="Remix social links" mix={socialNavCss}>
      ...
      <p mix={footerLegalCss}>DOCS AND EXAMPLES LICENSED UNDER MIT</p>
      <p mix={footerLegalCss}>&copy;2026 SHOPIFY, INC.</p>
    </footer>
  )
}
