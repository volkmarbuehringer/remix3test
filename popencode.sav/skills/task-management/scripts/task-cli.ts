...
function saveSubtask(feature: string, subtask: Subtask): void {
  let subtaskPath = path.join(TASKS_DIR, feature, `subtask_${subtask.seq}.json`);
...
function cmdStatus(feature?: string): void {
  let features = feature ? [feature] : getFeatureDirs();
  ...
  for (let f of features) {
    ...
    let progress = subtasks.length > 0
...
function cmdNext(feature?: string): void {
  ...
  for (let f of features) {
    let subtasks = loadSubtasks(f);
    let completedSeqs = new Set(subtasks.filter(s => s.status === 'completed').map(s => s.seq));
    ...
    if (ready.length > 0) {
      console.log(`[${f}]`);
      for (let s of ready) {
        let parallel = s.parallel ? '[parallel]' : '[sequential]';
...
function cmdParallel(feature?: string): void {
  ...
  for (let f of features) {
    ...
    let parallel = subtasks.filter(s => {
      if (s.status !== 'pending') return false;
...
function cmdDeps(feature: string, seq: string): void {
  let subtasks = loadSubtasks(feature);
  let target = subtasks.find(s => s.seq === seq);
  ...
  let printDeps = (seqs: string[], indent: string = '  '): void => {
    for (let i = 0; i < seqs.length; i++) {
      let depSeq = seqs[i];
      let dep = subtasks.find(s => s.seq === depSeq);
      let isLast = i === seqs.length - 1;
      let branch = isLast ? '└──' : '├──';

      if (dep) {
        let statusIcon = dep.status === 'completed' ? '✓' : dep.status === 'in_progress' ? '~' : '○';
...
function cmdBlocked(feature?: string): void {
  ...
  for (let f of features) {
    ...
    if (blocked.length > 0) {
      console.log(`[${f}]`);
      for (let s of blocked) {
        let waitingFor = s.depends_on.filter(dep => !completedSeqs.has(dep));
        let reason = s.status === 'blocked'
          ? 'explicitly blocked'
...
function cmdComplete(feature: string, seq: string, summary: string): void {
  ...
  let subtasks = loadSubtasks(feature);
  let subtask = subtasks.find(s => s.seq === seq);
  ...
  let task = loadTask(feature);
  if (task) {
    let newSubtasks = loadSubtasks(feature);
...
function cmdValidate(feature?: string): void {
  ...
  let validTaskStatuses = new Set(['active', 'completed', 'blocked', 'archived']);
  let validSubtaskStatuses = new Set(['pending', 'in_progress', 'completed', 'blocked']);

  let requiredTaskFields = [
    'id',
    'name',
    'status',
    'objective',
  ...
  let hasField = (obj: any, field: string): boolean => Object.prototype.hasOwnProperty.call(obj, field);
  let isStringArray = (value: any): boolean => Array.isArray(value) && value.every(v => typeof v === 'string');
  ...
  for (let f of features) {
    ...
    let subtasks = loadSubtasks(f);
    let seqCounts = new Map<string, number>();
    for (let s of subtasks) {
      let seq = typeof s.seq === 'string' ? s.seq : '';
      seqCounts.set(seq, (seqCounts.get(seq) || 0) + 1);
    }
    let seqs = new Set(subtasks.map(s => s.seq));

    if (task) {
      // Required fields in task.json
      for (let field of requiredTaskFields) {
    ...
    for (let s of subtasks) {
      ...
      for (let field of requiredSubtaskFields) {
        if (!hasField(s, field)) {
      ...
      for (let dep of (Array.isArray(s.depends_on) ? s.depends_on : [])) {
        if (!seqs.has(dep)) {
      ...
      let checkCircular = (seq: string, path: string[]): boolean => {
        ...
        let task = subtasks.find(t => t.seq === seq);
        if (task) {
