...
**Status**: Future consideration. Not started. Depends on Phase 1 (offerings-in-memory-token) proving the re-render-from-POST pattern works.
...
Remix 3's `remix/data-schema` supports `.refine()` for custom checks. The `timeboxer` demo demonstrates: `s.parseSafe(schema, formData)` → `issuesToErrors(issues)` → per-field errors. Phase 1 uses custom validation functions, which work but duplicate boilerplate. Phase 2 eliminates all of it.

Source: `node_modules/remix/src/data-schema/README.md:74-94` confirms `parseSafe` works with `f.object` schemas.
...
export let offeringSaveSchema = f
  .object({
    resource_id: f.field(
  ...
  .refine((v) => v.end_min > v.start_min, {
    message: 'muss nach der Startzeit liegen.',
    path: ['end_min'],
  })
...
if (!result.success) {
  let fieldErrors = fieldErrorsFromIssues(result.issues)
  let formValues = readOfferingFormValues(formData)
  let data = await loadOfferingPageData(context, { creating: true, formValues, fieldErrors })
...
- [ ] `app/utils/offering-schema.ts` — `f.object` schema for offerings
- [ ] `app/utils/appointment-schema.ts` — `f.object` schema for appointments
...
- [ ] `admin-appointments-form.tsx` — switch to receiving props from re-render (no more `fv_`/`fe_` decoding)
- [ ] `admin-users-controller.tsx`, `admin-resources-controller.tsx` — apply same pattern
- [ ] Verify all tests pass (error format stays `Record<string, string>`)

## Open Questions

- Does `.refine()` on `f.object` work the same as on `s.object`? The README shows `.refine()` on `s.object` but not `f.object`. Needs spike.
- Does `s.parseSafe()` with `f.object` return issues with `path` arrays matching field names? Needs verification.
- Can `coerce.number()` handle empty-string-to-NaN gracefully? `parseInt("", 10)` → `NaN`, but `coerce.number()` might reject empty strings.
- How to handle cross-field `endMin <= startMin` check? `.refine()` on whole object should work, but `path` targeting is unclear.
