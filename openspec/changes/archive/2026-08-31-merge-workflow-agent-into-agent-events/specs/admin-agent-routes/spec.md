...
The system SHALL serve the agent-events page under the `/admin` path prefix. The workflow-agent page is retired and its URL SHALL NOT resolve.

#### Scenario: Admin accesses /admin/workflow-agent

- **WHEN** an authenticated admin user navigates to the former `/admin/workflow-agent` path
- **THEN** the system SHALL NOT render a Workflow-Agent page at that path (404 or redirect)
...
- **WHEN** the panel frame content is requested at `/admin/agent-events/panel`
- **THEN** the system SHALL return the panel fragment
- **AND** the former `/admin/workflow-agent/panel` path SHALL NOT resolve (404 or redirect)
...
The system SHALL render the Agent-Events page inside the admin sidebar layout, not the public `MainNav` layout.

#### Scenario: Agent pages show the admin sidebar

- **WHEN** an admin user navigates to `/admin/agent-events`
- **THEN** the admin sidebar SHALL be visible
- **AND** the Agent-Events nav item SHALL be highlighted

#### Scenario: Non-admin is rejected from agent routes

- **WHEN** an authenticated non-admin user navigates to `/admin/agent-events`
- **THEN** the system SHALL return a 403 or redirect away

#### Scenario: Unauthenticated user is redirected from agent routes

- **WHEN** an unauthenticated user navigates to `/admin/agent-events`
- **THEN** the system SHALL redirect to the login page

### Requirement: Panel frames use unique names

The system SHALL name the nested panel frame `agent-events-panel` so sidebar frame navigation resolves to the page frame rather than the nested panel frame.

#### Scenario: Sidebar navigation from an agent page navigates the page frame

- **WHEN** an admin user clicks a sidebar nav item while on `/admin/agent-events`
...
- **THEN** the client SHALL navigate the panel frame named `agent-events-panel` to the event href