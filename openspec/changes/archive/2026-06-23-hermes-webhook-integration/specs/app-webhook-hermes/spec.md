...
- **THEN** the response SHALL contain `{ "id": "<uuid>", "delivery_id": "<hermes_id>" }` with status 200
...
- If hermes is unreachable, the API SHALL still return success (data is persisted) with a `hermes_status: "unreachable"` field
- On successful hermes delivery, the response SHALL include the `delivery_id` from hermes

#### Scenario: Hermes responds successfully

- **WHEN** the insert succeeds and hermes responds with `{ "delivery_id": "abc123" }`
- **THEN** the response SHALL include `{ "delivery_id": "abc123", "hermes_status": "accepted" }`

#### Scenario: Hermes is unreachable

- **WHEN** the insert succeeds but hermes does not respond within 3 seconds
- **THEN** the response SHALL still return 200 with `{ "id": "<uuid>", "hermes_status": "unreachable" }`
