...
- **Single resource-oriented action shape.** Replace `save`, `update`, `rename`, `data` with: `create` (POST `/`), `patch` (PATCH `/:id` accepting a *partial* body — `{ description? }`, `{ items? }`, or both), and keep `destroy`. **BREAKING** for any in-flight client calls; the only known consumer is this app's own client, which we are rewriting in the same change.
- **Fold loaded list into the frame response.** The `?load=N` GET returns the shell *and* the loaded list's data in the same response (server-rendered into the ` ListsClient ` initial state), eliminating the second `/lists/N/data` fetch. The `data` action is removed.
