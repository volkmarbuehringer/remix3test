...
#### Scenario: Changes validated on submit

- **WHEN** the admin submits an empty payload (all keys removed)
- **THEN** the system SHALL save `{}` as the payload
- **AND** the row SHALL update successfully

#### Scenario: Editing row is visually highlighted

- **WHEN** a row is being edited (`?editing=<id>` is active)
- **THEN** that row in the table SHALL have a highlighted border/background to indicate it is the active edit target
...
#### Scenario: Sort and filter persist with editing

- **WHEN** the admin has a filter active and sorts by a column, then clicks "Edit" on a row
- **THEN** the URL SHALL contain `?editing=<id>&offset=<n>&sort=<col>&order=<dir>&filter=<term>`
- **AND** the grid SHALL display the same page with the edit panel open
