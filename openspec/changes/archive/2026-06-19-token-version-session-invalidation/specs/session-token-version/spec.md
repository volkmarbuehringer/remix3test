...
The settings password change handler SHALL increment `users.token_version`, then re-read the user and re-issue the session auth with the new `tv` so the current device remains authenticated.
