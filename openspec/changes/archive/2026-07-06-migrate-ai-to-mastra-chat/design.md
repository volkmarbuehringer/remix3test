...
2. **Newer `/mastra/chat`** (`app/actions/mastra/controller.tsx`) — uses the Mastra `supportAgent` with `supportTools` (`lookup_user`, `list_recent_appointments`, `count_users`, `get_current_date_time`) and Postgres-backed `Memory` (`mastraStorage`). It currently _also_ double-writes to the legacy `chatlog` table via `createConversation`/`appendMessage`.
