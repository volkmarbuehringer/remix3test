...
Define the structural convention for organizing controller modules into feature-directories within `app/actions/`, where each feature's controller, tests, and page components are colocated in a single directory with `controller.tsx` as the canonical entry point.
