...
Mastra tools return their output to an LLM that must interpret the results. Without typed output schemas, the LLM infers field semantics from field names alone — leading to errors (e.g. dividing Unix ms by 1000, treating KiB as KB). This spec defines the requirements for typed output schemas with discriminated error unions, giving the LLM explicit field descriptions and a consistent error contract across all tools.
