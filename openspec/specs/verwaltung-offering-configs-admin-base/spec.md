# verwaltung-offering-configs-admin-base Specification

## Purpose

Defines how the `/verwaltung/offering-configs` CRUD page conforms to the shared admin page/form base contract so its create, update, and delete flows behave consistently with `/admin/users`, `/verwaltung/offerings`, and `/verwaltung/resources`: server-rendered frame-targeted row actions, Post/Redirect/Get mutations, validation failures re-rendered as a 200 inline-error fragment, non-field errors surfaced via redirect + `session.flash`, and grid-state round-trip — while keeping the dedicated `verwaltung` layout, the day/time rule editor, and the agent JSON create path.

## Requirements

### Requirement: Offering-configs mutations validate server-side and follow Post/Redirect/Get
The system SHALL validate every data-mutating offering-config submission (create, update, delete) server-side through the shared frame transport. On validation failure the controller SHALL re-render the targeted frame content at an OK (200) status carrying the per-field errors and the submitted field values. On success the controller SHALL redirect (Post/Redirect/Get) back to the grid with the current grid state preserved. The controller SHALL NOT use a non-OK response (such as 400, 404, or 409) to deliver a validation-failure re-render.

#### Scenario: Create validation failure re-renders the frame with inline errors
- **WHEN** an admin submits the create form with an invalid field (a missing or invalid resource, a start time not before the end time, a time value outside the valid range, or no day enabled)
- **THEN** the response is a 200 status containing the targeted frame fragment with the field error text or form-level banner rendered and the previously entered values still present
...
### Requirement: Offering-configs row actions are server-rendered and target the frame
...
### Requirement: Offering-configs client-driven enhancements are input affordances
...
### Requirement: Offering-configs non-field errors surface via redirect and flash
Non-field errors with no inline field (invalid row id, not-found row, a duplicate-config business-rule failure, and a delete blocked by a foreign-key constraint) SHALL be delivered by redirecting (Post/Redirect/Get) back to the grid and surfacing the message via `session.flash`. The controller SHALL NOT deliver these via a non-OK JSON response or a non-OK re-render.
...
- **THEN** the controller redirects (3xx) back to the grid and surfaces the block message via a flash message instead of re-rendering at a non-OK status

### Requirement: Offering-configs grid state is preserved across all mutations and navigations
...
### Requirement: Offering-configs empty state offers an explicit create entry point
When the grid has no rows and no create/edit form panel is active, the offering-configs page SHALL render a server-rendered frame-targeted `Neu anlegen` entry point so an empty list is not a dead end.
...
### Requirement: Offering-configs GET /:id resolves a deleted or missing row to the grid
