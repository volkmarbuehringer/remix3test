...
Serves the admin-only workflow-agent, agent-events, and support-agent pages under the `/admin` route tree, rendered with the admin sidebar chrome.
...
- **WHEN** an authenticated admin user navigates to `/admin/workflowagent2`
- **THEN** the system SHALL render the Agent-Events page
- **AND** the response SHALL have status 200

#### Scenario: Panel endpoints served under /admin

- **WHEN** the panel frame content is requested at `/admin/workflow-agent/panel` or `/admin/workflowagent2/panel`
- **THEN** the system SHALL return the panel fragment

#### Scenario: Old top-level paths no longer resolve

- **WHEN** an unauthenticated or authenticated user navigates to `/workflow-agent` or `/workflowagent2`
...
- **WHEN** an admin user navigates to `/admin/workflow-agent` or `/admin/workflowagent2`
...
- **WHEN** an authenticated non-admin user navigates to `/admin/workflow-agent` or `/admin/workflowagent2`
- **THEN** the system SHALL return a 403 or redirect away

#### Scenario: Unauthenticated user is redirected from agent routes

- **WHEN** an unauthenticated user navigates to `/admin/workflow-agent` or `/admin/workflowagent2`
...
- **WHEN** an admin user clicks a sidebar nav item while on `/admin/workflow-agent` or `/admin/workflowagent2`
