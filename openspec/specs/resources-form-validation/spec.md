...
Specifies form validation behavior for the admin resources management page at `/verwaltung/resources`. On validation failure, the controller re-renders the page directly with `formValues` and `fieldErrors` props, providing inline error display and field value preservation. Description SHALL have a minimum length of 8 characters.
