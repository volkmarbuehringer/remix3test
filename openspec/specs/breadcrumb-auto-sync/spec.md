...
**Purpose**: Ensure the centralized `ROUTE_LABELS` map covers all active routes so breadcrumbs render meaningful trails everywhere in the application.

## Requirements

### Requirement: All active routes have breadcrumb coverage

The centralized `ROUTE_LABELS` map at `app/ui/route-labels.ts` SHALL contain entries for every active route in the application, ensuring `getBreadcrumbs()` produces a meaningful trail on every page.
