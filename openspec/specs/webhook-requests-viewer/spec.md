...
The system SHALL render an SSR page at `/webhook-requests` displaying the contents of the `webhook_requests` table.
...
The page SHALL connect to an SSE endpoint and refresh the table when new data arrives.

#### Scenario: Auto-refresh on new request

- **WHEN** a new webhook request is received
- **THEN** the SSE endpoint SHALL push an event to connected clients
- **WHEN** the viewer page receives this event
- **THEN** the page SHALL re-fetch the current page data without full page navigation
