        ...
        <form method="GET" action={BASE} data-rmx-target={frames.adminContent} mix={table.filterBar}>
          <select name="year" mix={table.select}>
            {Array.from({ length: 5 }, (_, i) => {
              let y = curYear - 2 + i
              return (
                <option key={y} value={y} selected={y === curYear}>
                  {y}
                </option>
              )
            })}
          ...
          <select name="user_id" mix={table.select}>
            ...
            {p.users.map((u) => (
              <option key={u.id} value={u.id} selected={curUserId === Number(u.id)}>
                {u.name}
              </option>
            ))}
          </select>
          ...
          {(curFilter ||
            ...
            curMonth !== new Date().getUTCMonth() + 1 ||
            curUserId !== undefined) && (
                ...
                {[
                  ...
                  <th key={field} mix={table.thSortable}>
                    <a
                      href={buildUrl({
                        sort: field,
                        order: field === curSort ? (curOrder === 'asc' ? 'desc' : 'asc') : 'asc',
                        offset: '0',
                        year: String(curYear),
                        month: String(curMonth),
                        selectedUserId: curUserId !== undefined ? String(curUserId) : undefined,
