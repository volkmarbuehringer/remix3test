...
>(routes.mastra.chat, {
  ...
  actions: {
    ...
    async toolDecision(context) {
      ...
      let toolCallId =
        context.formData.get("toolCallId")?.toString() || undefined;
          ...
          try {
            ...
            if (result.finishReason === "suspended") {
              ...
              if (sp?.question) {
                ...
                controller.enqueue(
                  sseEncoder.encode(`event: complete\ndata: {}\n\n`),
                );
                controller.close();
                return;
              }
            ...
            let text = (
              result.text ||
              (decision === "approve" ? "" : "Die Aktion wurde abgelehnt.")
            ).trim();
            if (text) {
              controller.enqueue(
                sseEncoder.encode(
