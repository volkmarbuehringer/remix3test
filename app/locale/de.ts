  ...
  verification: {
    ...
    html(name: string, url: string) {
      return [
        ...
        `<p>Hallo ${esc(name)},</p>`,
        '<p>vielen Dank für die Erstellung Ihres Kontos! Bitte bestätigen Sie Ihre E-Mail-Adresse, indem Sie auf den folgenden Link klicken:</p>',
  ...
  passwordReset: {
    ...
    html(name: string, url: string) {
      return [
        ...
        `<p>Hallo ${esc(name)},</p>`,
        '<p>wir haben eine Anfrage zum Zurücksetzen Ihres Passworts erhalten. Klicken Sie auf den folgenden Link, um ein neues Passwort zu vergeben:</p>',
        `<p><a href="${esc(url)}">${esc(url)}</a></p>`,
    ...
    self: {
      html(name: string) {
        return [
          ...
          `<p>Hallo ${esc(name)},</p>`,
          '<p>Ihr Konto wurde erfolgreich gelöscht. Diese Aktion kann nicht rückgängig gemacht werden.</p>',
          '<p>Wenn Sie Fragen haben oder Hilfe benötigen, wenden Sie sich bitte an unseren Support.</p>',
        ].join('\n')
      },
      text(name: string) {
        return [
