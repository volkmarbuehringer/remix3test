...
import { insertAppWebhookRequest, updateHermesStatus } from './app-webhook.ts'
...
describe('app-webhook', () => {
  ...
  it('updateHermesStatus updates the status for an existing row', async () => {
    ...
    await updateHermesStatus(db, testId, 'processed')
  ...
  it('updateHermesStatus does not throw for non-existent id', async () => {
    await updateHermesStatus(db, '00000000-0000-0000-0000-000000009999', 'ignored')
  })
})
