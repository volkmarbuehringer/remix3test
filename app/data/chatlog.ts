...
export async function createConversation(db: Database, userId: number): Promise<string> {
  let now = Date.now()
  let attempts = 0
  let maxAttempts = 3
...
export async function getConversation(db: Database, id: string, userId?: number): Promise<ChatLogRow | null> {
...
export async function appendMessage(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    let existing = await getConversation(db, id, userId)
    if (!existing) {
      return null
    }
...
export async function getAllConversations(
  ...
  if (filter && filter.trim()) {
    let esc = filter.trim().slice(0, 200).replace(/[%_\\]/g, '\\$&')
    if (type === 'agent') {
      result = await db.exec(sql`
        SELECT * FROM chatlog
        WHERE conversation::text ILIKE '%' || ${esc} || '%'
...
export async function deleteConversation(db: Database, id: string, userId?: number): Promise<void> {
  if (userId != null) {
