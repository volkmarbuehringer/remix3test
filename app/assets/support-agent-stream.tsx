    ...
    function showSuspension(data: {
      ...
      if (isCancelUser) {
        ...
        approveBtn.style.background =
          "var(--rmx-color-action-primary-background, #0066cc)";
        approveBtn.style.color =
          "var(--rmx-color-action-primary-foreground, #fff)";
      }
      ...
      declineBtn.style.border =
        "1px solid var(--rmx-color-border-default, #ccc)";
    ...
    async function startStream(url: string, init: RequestInit) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          let msg = match
            ? (JSON.parse(match[1]).error ?? res.statusText)
            : res.statusText;
              ...
              if (eventType === "start") {
                ...
                if (!didNavigate) {
                  let container = document.getElementById(
                    "support-agent-frame-container",
                  );
                  let activeFrame =
                    container?.getAttribute("data-active-frame") ??
                    "admin-content";
