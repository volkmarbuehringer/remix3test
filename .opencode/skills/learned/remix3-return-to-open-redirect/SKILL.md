---
name: remix3-return-to-open-redirect
description: "Use when sanitizing a `returnTo` redirect target — an origin-only check still allows a same-origin path that normalizes to `//host` (open redirect)."
