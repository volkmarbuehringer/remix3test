...
description: "Use when styling a remix3 app with theme tokens — raw `var(--rmx-...)` strings, bare variable names, or a `*.background` token used as a text color silently break light/dark contrast."
origin: consolidated
---
...
Styling drifts out of the theme in five ways, all of which skip the contract and break dark mode:
...
5. **A `*.background` fill token used as a text `color`.** `success.background` / `warning.background` are pale tints in light mode and near-black in dark mode, so as ink they are invisible in **both** themes; `danger.background` is a saturated red that happens to read fine, so the bug is easy to inherit from danger styling. Both `tsc` and `check-theme-conformance` pass it.
