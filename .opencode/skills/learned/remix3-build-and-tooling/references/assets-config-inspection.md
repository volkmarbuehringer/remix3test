...
**Source:** `remix3-assets-config-inspection`
...
6. **(#11814, 2026-09-08)** The asset server now auto-mounts the package-manager virtual store at `/__@remix/virtual-store` when it lives **outside** `rootDir` (see `~remix/packages/assets/src/lib/virtual-store.ts`). It only fires when the store is outside every mount — **not for this app**, whose pnpm `.pnpm` sits inside the `node_modules` mount (`isWithinMounts` returns `[]`). No config change needed here; just be aware `remix assets` may list `/__@remix/virtual-store/*` paths for apps whose store is external.
