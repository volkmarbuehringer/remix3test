---
name: remix3-aria-tabs
description: "Use when a Remix 3 page has several same-page sections that should show one at a time (settings/profile tabs), or when a section nav must deep-link via the URL hash and support Arrow/Home/End — build ARIA tabs in a `clientEntry` over `hidden` panels with a no-JS stacked fallback."
...
1. **`hidden` doesn't hide.** The shared panel descriptor sets `display:flex`,
   ...
   `panel.hidden = true` does nothing. See `remix3-hidden-attribute-display-override`.
