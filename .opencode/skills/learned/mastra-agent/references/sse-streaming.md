# Streaming via SSE (Single-Request Pattern)

## What This Covers

Streaming `agent.stream()` output over SSE in one POST request. Read this when replacing the two-connection POST + EventSource protocol, adding `askUserTool` questions, or resuming a snapshot-suspended agent.

- Server controller, `pipeStream` helper, and SSE headers
- Client `fetch()` + `getReader()` consumer and the SSE event contract
- `askUserTool` suspension vs `requireApproval`
- The `requireToolApproval` snapshot loss on resume

For tool-result extraction, see `tool-results.md`. For model construction, see `model-config.md`.
...
Suspect this issue when approval works on the first tool call but is silently bypassed on subsequent calls, the tool executes without the expected prompt after approving/declining, or the resumed run permits tool calls that were previously gated.
