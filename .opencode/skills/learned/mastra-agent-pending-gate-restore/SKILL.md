---
name: mastra-agent-pending-gate-restore
description: "Use when a Mastra agent's ask_user question or tool-approval card is missing after a reload, or a suspended run is orphaned so the user must retype — persist a per-actor pending-gate payload and re-surface it via a reconnect endpoint."
metadata:
